...
• 在所有满足条件的组合中随机抽取 top_n 条，并按分差从小到大排序输出
用法示例：
...
def build_pairs(entries, score_gap):
    """生成所有满足分差阈值的 (chosen, rejected) 组合，并附上分差"""
    results = []
...
def main():
    ...
    # 按照 score_diff 从小到大排序
    sorted_sampled = sorted(sampled, key=lambda x: x["score_diff"])

    # 输出前移除 score_diff 字段，仅保留 chosen 和 rejected
    final_output = [{"chosen": item["chosen"], "rejected": item["rejected"]} for item in sorted_sampled]
    write_jsonl(final_output, args.output)
