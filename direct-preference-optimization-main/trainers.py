...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        """A trainer for a language model supporting SFT or DPO training."""
        self.seed = seed
        self.rank = rank
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        """Generate samples from the policy (and reference model if applicable) for the given batch."""
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        """Compute loss and other metrics for the given batch."""
        metrics = {}
    ...
    def train(self):
        """Begin training with periodic evaluation and logging."""
        ...
        for batch in self.train_iterator:
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                rank0_print(f'Eval after {self.example_counter} examples: {formatted_dict(mean_eval_metrics)}')
                # 保存评估时的 checkpoint（非 debug 模式下）
                if self.example_counter > 0:
                    if self.config.debug:
                        rank0_print('Skipping checkpoint save in debug mode.')
                    else:
                        output_dir = os.path.join(self.run_dir, f'step-{self.example_counter}')
                        rank0_print(f'Creating checkpoint in {output_dir} ...')
            ...
            if last_log is None or time.time() - last_log > self.config.minimum_log_interval_secs:
                ...
                rank0_print(f"Skipping logging after {self.example_counter} examples (logging interval not reached)")
            # ========= END TRAINING =========

    def clip_gradient(self):
        """Clip the gradient norm of the parameters (for non-FSDP policy)."""
...
class FSDPTrainer(BasicTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        """A trainer that uses PyTorch FSDP to shard the model across GPUs."""
        super().__init__(policy, config, seed, run_dir, reference_model, rank, world_size)
        assert config.model.block_name is not None, 'must specify model.block_name for FSDP'
        ...
        if config.activation_checkpointing:
            ...
            try:
                ...
                rank0_print('Applying activation checkpointing wrapper...')
                apply_activation_checkpointing(self.policy, checkpoint_wrapper_fn=non_reentrant_wrapper, check_fn=check_fn)
                rank0_print('Activation checkpointing enabled.')
        if config.loss.name in {'dpo', 'ipo'}:
            rank0_print('Sharding reference model with FSDP...')
            self.reference_model = FSDP(reference_model, **shared_fsdp_kwargs)

        rank0_print('Loaded model on rank', rank)
        dist.barrier()

    def clip_gradient(self):
        """Clip the gradient norm for FSDP policy by gathering gradients across GPUs."""
        return self.policy.clip_grad_norm_(self.config.max_grad_norm).item()

    def save(self, output_dir=None, metrics=None):
        """Save policy, optimizer, and scheduler state to disk (only on rank 0)."""
