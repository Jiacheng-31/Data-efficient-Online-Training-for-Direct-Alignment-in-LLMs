...
BASE_FILE="DPO/dataset/hh-train.jsonl"

DecoderLayer=LlamaDecoderLayer
MODEL_NAME=llama3-8b
EXP_NAME="$MODEL_NAME-dataset-hh-MAB-dpo-start-t0.7-0.3-ref-sft"
SAMPLE_NUM=1000
SAMPLE_TIMES=12
TOP_N=6000
MODEL_LOR="Llama3-8B-Base"
NUM_RESPONSE=3
REWARDMODEL="llama3-8b-reward"

BASE_WORKSPACE="DPO/MAB/$EXP_NAME"
mkdir -p $BASE_WORKSPACE
cp -r DPO/turn/llama3-8b-dpo-test5 $BASE_WORKSPACE
REF_MODEL_LOR="DPO/turn/llama3-8b-sft-test5"
EXP_MODEL_LOR="$BASE_WORKSPACE/llama3-8b-dpo-test5"

CLUSTER_DIR="DPO/MAB/k-means-clusters-hh"
...
for round in {1..3}; do
  ...
  for iter in {1..8}; do
    ...
    torchrun --nproc_per_node=$NGPU DPO/data_cluster/prompt/torch_run_model.py \
    ...
    bash DPO/Reward/run_reward_scoring.sh \
    ...
    torchrun --nproc_per_node=$NGPU DPO/data_select/PPL-NEW.py \
    ...
    python DPO/MAB/update_cluster_score.py \
  ...
  python DPO/data_select/dataselect-random.py \
  ...
  cd DPO/direct-preference-optimization-main
